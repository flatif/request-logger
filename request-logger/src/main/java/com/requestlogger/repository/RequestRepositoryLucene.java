...
public class RequestRepositoryLucene implements RequestRepository {
	...
	public RequestRepositoryLucene() {
		try {
			...
			throw new RuntimeException("Can't initialize lucene index writer", e);
		}
	}
	...
	private List<HttpRequest> convert(TopDocs topDocs) throws IOException{
		final LinkedList<HttpRequest> result = new LinkedList<HttpRequest>();
	...
	private HttpRequest convert(ScoreDoc doc) throws IOException {
